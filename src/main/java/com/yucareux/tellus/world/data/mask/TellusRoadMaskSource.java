...
public final class TellusRoadMaskSource {
    ...
    private static final String DEFAULT_SOURCE = "tellus_roadmask.pmtiles";
    private static final int MAX_CACHE_TILES = intProperty("tellus.roadmask.cacheTiles", 256);

    private final PmTilesReader reader;
    private final LoadingCache<TileKey, @Nullable RoadMaskTile> cache;
    private final int minZoom;
    private final int maxZoom;
    private final boolean available;

    public TellusRoadMaskSource() {
        String source = System.getProperty("tellus.roadmask.source", DEFAULT_SOURCE);
        this.reader = new PmTilesReader(source);
        int resolvedMin = 0;
        int resolvedMax = 0;
        boolean ok = false;
        try {
            ...
            Tellus.LOGGER.warn("Road mask PMTiles unavailable, roads will not be generated", e);
        }
        this.available = ok;
        ...
        this.cache = CacheBuilder.newBuilder()
                ...
                .build(new CacheLoader<>() {
                    @Override
                    public @Nullable RoadMaskTile load(TileKey key) throws Exception {
                        return TellusRoadMaskSource.this.loadTile(key);
                    }
                });
    }

    public RoadMaskSample sampleRoads(double blockX, double blockZ, double worldScale) {
        if (!this.available || worldScale <= 0.0) {
            return RoadMaskSample.unknown();
        }
        ...
        RoadMaskTile tile = getTile(key);
        if (tile == null) {
            return RoadMaskSample.unknown();
        }

        if (tile.isEmpty()) {
            return RoadMaskSample.known(false);
        }
        ...
        return RoadMaskSample.known(tile.isRoad(px, py));
    }
    ...
    private RoadMaskTile getTile(@NonNull TileKey key) {
        try {
            return this.cache.get(key);
        } catch (Exception e) {
            Tellus.LOGGER.debug("Failed to load road mask tile {}", key, e);
            return null;
        }
    }

    private RoadMaskTile loadTile(TileKey key) throws IOException {
        ...
        if (bytes == null) {
            return RoadMaskTile.empty();
        }
        BufferedImage image = ImageIO.read(new ByteArrayInputStream(bytes));
        if (image == null) {
            throw new IOException("Invalid road mask tile image");
        }
        ...
        return new RoadMaskTile(width, height, mask, false);
    }
    ...
    public record RoadMaskSample(boolean known, boolean road) {
        public static RoadMaskSample known(boolean road) {
            return new RoadMaskSample(true, road);
        }

        public static RoadMaskSample unknown() {
            return new RoadMaskSample(false, false);
        }
    }
    ...
    private static final class RoadMaskTile {
        private static final RoadMaskTile EMPTY = new RoadMaskTile(0, 0, new byte[0], true);
        ...
        private RoadMaskTile(int width, int height, byte[] mask, boolean empty) {
            this.width = width;
            this.height = height;
            this.mask = mask;
            this.empty = empty;
        ...
        public static RoadMaskTile empty() {
            return EMPTY;
        }
